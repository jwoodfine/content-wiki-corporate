---
title: "Requisitos de presentación del MD&A trimestral"
slug: quarterly-mda-filing-requirements
category: governance
last_edited: 2026-06-29
editor: woodfine-editorial
status: stable
references:
  - id: 1
    text: "National Instrument 51-102 Continuous Disclosure Obligations. CSA, 2004 (as amended)."
  - id: 2
    text: "NIC 34 Información Financiera Intermedia. IASB."
---

Los [[regulated-reporting-entity|emisores canadienses que presentan informes]] sujetos a la [[continuous-disclosure-obligations|NI 51-102]] están obligados a
depositar estados financieros intermedios y un Análisis y Discusión de la Administración
(MD&A) trimestral para cada uno de los tres primeros trimestres fiscales. El ciclo de
informes trimestrales se distingue del ciclo anual tanto por la norma de estados financieros
aplicada como por el plazo aplicable al depósito.

## Estados financieros intermedios

Los estados financieros trimestrales presentados bajo la NI 51-102 se preparan de
conformidad con la NIC 34 Información Financiera Intermedia en lugar de las normas NIIF
completas aplicadas a los estados anuales.[^2] La NIC 34 permite un formato condensado que
incluye:

- Un estado condensado de situación financiera
- Un estado condensado de resultado integral para el trimestre y el período acumulado del
  año hasta la fecha, con cifras comparativas del año anterior
- Un estado condensado de cambios en el patrimonio neto
- Un estado condensado de flujos de efectivo
- Notas explicativas seleccionadas

## Requisitos del MD&A trimestral

Los elementos requeridos incluyen:

- **Resultados de las operaciones** — discusión de los resultados del trimestre actual y del
  acumulado hasta la fecha en comparación con los períodos comparativos del año anterior
- **Liquidez y recursos de capital** — análisis de los flujos de efectivo del período actual
- **Acuerdos fuera de balance** — divulgación actualizada de cualquier [[material-change-reporting|cambio material]]
- **Transacciones con partes relacionadas** — divulgación completa
- **Transacciones propuestas** — divulgación de adquisiciones o enajenaciones significativas
  ya anunciadas públicamente pero aún no completadas

Para las entidades inmobiliarias, el MD&A trimestral típicamente aborda la ocupación de la
cartera y las tendencias de las tasas de alquiler, los movimientos del valor razonable de las
propiedades de inversión, el [[debt-service-and-financing-structure|perfil de vencimiento de la deuda]] y la exposición a tipos de
interés, y cualquier cambio material en la [[direct-hold-solutions-structural-comparison|estructura de la cartera de tenencia directa]].

## Plazos de presentación

Los emisores que presentan informes no clasificados como empresas de riesgo deben depositar
los estados financieros trimestrales y el MD&A en un plazo de **45 días naturales** a partir
del final de cada uno de los tres primeros trimestres fiscales.[^1] Los emisores clasificados
como empresas de riesgo disponen de **60 días naturales** para los depósitos trimestrales.

## Véase también

- [[ni-51-102-annual-reporting-cycle]] — las obligaciones de informes anuales que completan
  el ciclo del ejercicio fiscal
- [[material-change-press-release-protocol]] — la divulgación impulsada por eventos
  desencadenada por cambios materiales entre fechas de presentación
- [[sedar-electronic-filing-standards]] — los requisitos del sistema de presentación
  electrónica
